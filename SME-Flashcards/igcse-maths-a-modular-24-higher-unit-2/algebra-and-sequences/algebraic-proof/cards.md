...
Course: igcse-maths-a-modular-24-higher-unit-2 · Section: Algebra & Sequences

Source: https://www.savemyexams.com/igcse/maths/edexcel/a-modular/24/higher-unit-2/flashcards/algebra-and-sequences/algebraic-proof/


## Card 1 — keyword_definition (`fl_6dJYgFYqqFgr6Wwd`)
...
$2n$, for example, where $n$ is an integer, is an expression for an even integer.

Other letters could also be used.



## Card 2 — keyword_definition (`fl_6hCWcmZMRSHkf47V`)
...
$2n+1$, for example, where $n$ is an integer, is an expression for an odd integer.

Another possibility is $2n-1$.



## Card 3 — keyword_definition (`fl_2CKQ6xcVDRd4bRtb`)
...
$7n$, for example, where $n$ is an integer, is an expression for an integer that is a multiple of 7.



## Card 4 — keyword_definition (`fl_PPWBBMbXH4424hT5`)
...
## Card 5 — keyword_definition (`fl_CHgGPKQqqN2Tjm6C`)
...
$2n$ and $2m$, for example, where $n$ and $m$ are different integers, are expressions for any two even numbers.

It is not $2n$ and $2n+2$, as this would mean two consecutive even numbers.



## Card 6 — question_and_answer (`fl_jWdwMSFpycFxX3V6`)
...
## Card 7 — true_or_false (`fl_gwztHkV7D8jsczSN`)
...
It has the form of an odd number, `Error converting from MathML to accessible text.`, as $n^{2}+n$ is an integer.



## Card 8 — true_or_false (`fl_K8NjC6DtyB2TMWkR`)
...
The problem with `2 open parentheses n plus 1 half close parentheses` is that the part inside the brackets has a $\frac{1}{2}$ in it, so would never be an integer.
...
## Card 9 — question_and_answer (`fl_RxBBsKFZpJghCZ9Y`)
...
$3n$ and $3n+3$ are two consecutive multiples of 3.
...
## Card 10 — question_and_answer (`fl_jrVQjJZwQrWWvp7t`)

**FRONT**

How could you show that $8n+5$ is odd (assuming $n$ is an integer)?
...
One way show that $8n+5$ is odd is to rewrite $8n+5$ as $8n+4+1$ then factorise out a 2 to get `2 open parentheses 4 n plus 2 close parentheses plus 1`.
...
## Card 11 — true_or_false (`fl_ZPXHqBX2ZZqgbF4h`)

**FRONT**

**True or False?**

If $p$ is a prime number and $p=ab$ where $a$ and $b$ are positive integers, then $a=1$ and $b=p$.
...
If $p$ is a prime number and $p=ab$ where $a$ and $b$ are positive integers, then $a=1$ and $b=p$ is **one possible **answer.
