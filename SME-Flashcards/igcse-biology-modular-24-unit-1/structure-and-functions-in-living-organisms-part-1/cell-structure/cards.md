# Cell Structure

Course: igcse-biology-modular-24-unit-1 · Section: 2. Structure & Functions in Living Organisms: Part 1

Source: https://www.savemyexams.com/igcse/biology/edexcel/modular/24/unit-1/flashcards/structure-and-functions-in-living-organisms-part-1/cell-structure/


## Card 1 — question_and_answer (`fl_9GDyF3y3k5FH98Tt`)

**FRONT**

What **separates **cells from their surrounding environment?


**BACK**

The **cell membrane **separates** **cells from their surrounding environment.



## Card 2 — question_and_answer (`fl_vynBjTpvtdq2bPgR`)

**FRONT**

Name the **largest organelle** present within animal cells.


**BACK**

The largest organelle within animal cells is the **nucleus**.



## Card 3 — question_and_answer (`fl_C4byVqs4k3Cnr6cS`)

**FRONT**

Name the structures **present in plant cells** that are *not* present in animal cells.


**BACK**

**Plant cells** have the following structures that are **not present in animal cells**:

- Cell wall
- Chloroplasts
- Permanent vacuole



## Card 4 — true_or_false (`fl_yJsb9JbvCPDWKvKV`)

**FRONT**

**True or False? **

Ribosomes are present in both plant and animal cells.


**BACK**

**True**.

Ribosomes are **present in plant and animal cells**. They are the site of protein synthesis.



## Card 5 — question_and_answer (`fl_WJyPnw9nrvfycDCs`)

**FRONT**

What is the function of the **cell membrane**?


**BACK**

The cell membrane **controls** the** **passage of substances** in and out of the cell**.



## Card 6 — keyword_definition (`fl_dDyYZ5x2DKQVpTK9`)

**FRONT**

Name **three organelles** found **within** animal cells.


**BACK**

Organelles present within** animal cells** are:

- Nucleus
- Mitochondria
- Ribosomes

Note that structures such as the cell membrane and cytoplasm are not **organelles**, so are not acceptable answers here.



## Card 7 — keyword_definition (`fl_76DZ2MVrmW5x4H6r`)

**FRONT**

What is the **smallest organelle** present in both plant and animal cells?


**BACK**

**Ribosomes** are the smallest organelle; they are present in both plant and animal cells.



## Card 8 — true_or_false (`fl_G7TMRVdsxJnvhGgZ`)

**FRONT**

**True or False?**

The gel-like fluid present inside both plant and animal cells is the chloroplast.


**BACK**

**False.**

The  gel-like fluid present in plant and animal cells is the **cytoplasm**. *Chloroplasts* are organelles found in plant cells.



## Card 9 — true_or_false (`fl_q9kgzwVbSjD3p5rd`)

**FRONT**

**True or False?**

The role of the nucleus is to release energy for the cell.


**BACK**

**False. **

The role of the nucleus is to **store genetic information** in the form of **DNA**. Energy is released by the *mitochondria*.



## Card 10 — question_and_answer (`fl_fybTC3KcwR4kSt8P`)

**FRONT**

What is the role of cell **cytoplasm**?


**BACK**

Cytoplasm is the **site of chemical reactions** inside cells.



## Card 11 — true_or_false (`fl_DCDkZxsT86fHDrNC`)

**FRONT**

**True or False?**

Plant cell walls are responsible for controlling the passage of substances in and out of cells.


**BACK**

**False.**

Plant cell walls provide **strength** and additional **structural support **for plant cells. It is the *membrane* that regulates the passage of substances.



## Card 12 — true_or_false (`fl_NmXshtQH94QPhsWS`)

**FRONT**

**True or False?**

Muscle cells are likely to contain more mitochondria than skin cells.


**BACK**

**True.**

Muscle cells are likely to contain more mitochondria than skin cells. This is because they are more metabolically active, so will need **more energy from respiration**, the process that occurs inside mitochondria.



## Card 13 — true_or_false (`fl_R9q8sQZSBpkCqG4v`)

**FRONT**

**True or False?**

Mitochondria are the site of aerobic respiration.


**BACK**

**True**.

Mitochondria are responsible for **releasing energy** through the process of **aerobic respiration**.



## Card 14 — true_or_false (`fl_dS2Tm6GX582zrbsg`)

**FRONT**

**True or False? **

Ribosomes are the site of protein synthesis.


**BACK**

**True. **

Ribosomes are responsible for **protein synthesis**, the process by which proteins are made.



## Card 15 — question_and_answer (`fl_gddH4F4FthVBxRSY`)

**FRONT**

What is the function of **chloroplasts **within plant cells?


**BACK**

Chloroplasts **absorb light energy** during the process of **photosynthesis**.



## Card 16 — question_and_answer (`fl_Fth9CcskN7D3PkYc`)

**FRONT**

What is the function of the **permanent vacuole** in plant cells?


**BACK**

The permanent vacuole **stores** **cell sap** and provides **structural support** for plant cells.



## Card 17 — true_or_false (`fl_kGSQt3gShmnqBHkk`)

**FRONT**

**True or False?**

Animal cells have mitochondria, ribosomes and a cell wall.


**BACK**

**False**.

Animal cells contain mitochondria and ribosomes but **do not have cell walls**. Cell walls are a feature of *plant *cells.



## Card 18 — question_and_answer (`fl_yZKmz2kWDbYJR7h5`)

**FRONT**

Give **three differences** between plant and animal cells.


**BACK**

Three **differences** between plant and animal cells are:

- Plant cells contain chloroplasts while animal cells do not
- Plant cells have a cell wall while animal cells do not
- Plant cells have a permanent vacuole while animal cells do not



## Card 19 — question_and_answer (`fl_njqkBspZ8QFxDfwk`)

**FRONT**

Describe the **permanent vacuole**.


**BACK**

The permanent vacuole in plant cells is a membrane-bound structure that **stores cell sap **and** helps maintain cell shape**.



## Card 20 — true_or_false (`fl_42JKRbKChR6y5bzN`)

**FRONT**

**True or False? **

Mitochondria** **are the site of photosynthesis in plant cells.


**BACK**

**False. **

Mitochondria are the site of (aerobic) respiration. *Chloroplasts* are the site of photosynthesis.



## Card 21 — true_or_false (`fl_GF4G8MyMhgGDh4pd`)

**FRONT**

**True or False?**

Cell membranes provide structural support to plant and animal cells.


**BACK**

**False.**

The cell membrane does not have a rigid structure so does *not *provide any structural support. Its role is to **control what enters and leaves the cell**.



## Card 22 — question_and_answer (`fl_QRDNS9Wk3mgCCmdJ`)

**FRONT**

Which cellular structure **contains the genetic material** in both plant and animal cells?


**BACK**

The** nucleus **contains the genetic material, DNA, in both plant and animal cells.



## Card 23 — question_and_answer (`fl_cWJRjc3pgpBr6NWj`)

**FRONT**

List the structural features that are found in **both plant and animal cells**.


**BACK**

Structural features present in **both plant and animal cells **are:

- Cell membrane
- Cytoplasm
- Nucleus
- Mitochondria
- Ribosomes



## Card 24 — true_or_false (`fl_sc5fSJFZhmWZdqn7`)

**FRONT**

**True or False?**

Plant cells do not contain mitochondria.


**BACK**

**False**.

Plant cells *do* contain mitochondria. Mitochondria are the site of aerobic respiration, a process that is essential for both plant and animal cells.



## Card 25 — question_and_answer (`fl_jVjGrvTCMfSQcBm3`)

**FRONT**

What is **cell differentiation**?


**BACK**

Cell differentiation is the process by which a cell changes to become **specialised**.



## Card 26 — true_or_false (`fl_ychM6fhYSgH9kxF5`)

**FRONT**

**True or False? **

Almost all cells in a multicellular organism contain different genetic information.


**BACK**

**False. **

Almost all cells in a multicellular organism contain the **same **genetic information.



## Card 27 — keyword_definition (`fl_fhSt2ymStBgxFrTd`)

**FRONT**

Define the term **specialised cell**.


**BACK**

Specialised cells are cells that have developed **specific characteristics **that allow them to perform **particular functions**.



## Card 28 — question_and_answer (`fl_KB8mrjYKGQDJ4CTj`)

**FRONT**

What **determines the features** of specialised cells?


**BACK**

The features of specialised cells are determined by the **expression of genes** in the nucleus.



## Card 29 — question_and_answer (`fl_VgCqvz8xFMXwRDyp`)

**FRONT**

How are **red blood cells** specialised to carry out their function?


**BACK**

Red blood cells have a **large surface area** to increase diffusion of oxygen and contain **haemoglobin** which binds to and transports oxygen. They have **no nucleus** to allow more space for haemoglobin.



## Card 30 — keyword_definition (`fl_whXYRZdRCRQ5fwrD`)

**FRONT**

Give an example of a **specialised plant cell**.


**BACK**

Examples of **specialised plant cells **include:

- Root hair cell
- Palisade mesophyll cell
- Xylem vessel



## Card 31 — true_or_false (`fl_bvQJ99dJ3G4xT4RC`)

**FRONT**

**True or False?**

Root hair cells have an increased surface area.


**BACK**

**True.**

Root hair cells have an** increased surface area** to allow **increased absorption** of water and mineral ions from the soil.



## Card 32 — keyword_definition (`fl_D5zfwRnp5ZKXhbXZ`)

**FRONT**

Give an example of a **specialised animal cell**.


**BACK**

Examples of **specialised animal cells** include:

- Red blood cells
- Nerve cells
- Ciliated epithelial cells
- Sperm / egg cells
- Muscle cells



## Card 33 — keyword_definition (`fl_8BRbfk38Myt3v8bP`)

**FRONT**

Define the term **stem cell**.


**BACK**

A stem cell is an **undifferentiated cell **that can differentiate to form **other cell types **and that can **divide an unlimited number of times**.



## Card 34 — true_or_false (`fl_9x5bwKn2dn8987fK`)

**FRONT**

**True or False? **

Embryonic stem cells can differentiate to produce most cell types.


**BACK**

**True**.

Embryonic stem cells can differentiate into most, or in some cases all, cell types. This enables them to **form the tissues and organs** needed during development.



## Card 35 — question_and_answer (`fl_7ZK66dH2mG3xzvmD`)

**FRONT**

What are the potential applications of stem cells in medicine?


**BACK**

Stem cells could potentially **cure diseases** such as diabetes and paralysis, and **repair **or** replace damaged organs**.



## Card 36 — true_or_false (`fl_Dg4FxkMfjSRdFfKJ`)

**FRONT**

**True or False?**

There is risk from infection or mutation when transferring lab-cultured stem cells into a patient.


**BACK**

**True.**

Stem cells cultured in a lab could become **infected**, or could accumulate **mutations**. These could be **passed to the patient** during therapy.



## Card 37 — keyword_definition (`fl_6dvrfqsVMBN638Rf`)

**FRONT**

What is the potential benefit of being able to use a **patient's own stem cells** in medical treatment?


**BACK**

A potential benefit of being able to use a patient's own stem cells in medical treatment is that stem cells from the same patient would **not be rejected by **the patient's** immune system**.



## Card 38 — true_or_false (`fl_mY3NDyWwD9mjVcJY`)

**FRONT**

**True or False?**

There are ethical issues associated with the use of embryonic stem cells in medicine.


**BACK**

**True.**

Many have ethical concerns around the use of embryonic stem cells in medicine. These concerns are related to whether or not embryos should have **human rights**. Concerns include:

- Destruction of embryos
- Embryos cannot give consent
- Embryos are treated as a commodity



## Card 39 — true_or_false (`fl_q4ZJrzVbyd6DpB7n`)

**FRONT**

**True or False?**

Adult stem cells have a limited capacity to differentiate.


**BACK**

**True.**

Adult stem cells have a **limited ability to differentiate**; they can only give rise to a few cell types.


