#!/usr/bin/env python3
"""T-C11 session 66 — c11_batch11_boundary_check.py: standing checker for the
cross-slice boundary ruling (scripts/c11_batch11_boundary_ruling.yaml).

Machine-verifies the ruling's claims against the LIVE store so the ruling is
checkable, not prose (the c11_batch10_boundary_check.py pattern):

  A. ruling schema          — sections present, session/date/attribution
  B. conflict audit         — re-running the S4-g/h candidate-term match
                              against the PRE-batch-11 store (registry minus
                              the batch-11 record when it exists, the live
  ...
  D. discipline invariants  — the ruling mints nothing beyond the sanctioned
                              batch-11 authoring (checked post-authoring), the
                              non_mint_list is duplicate-free + covered, the
                              semantic HV count is the operator promotions
                              (255; PART_OF HV rides the later T-C19 G19
                              record)

Usage: python3 scripts/c11_batch11_boundary_check.py
"""
...
OWNERS = {
    ...
    "batch 9": HERE / "c11_batch9_decisions.yaml",
    "batch 10": HERE / "c11_batch10_decisions.yaml",
}
...
check("A1 ruling parses with required sections",
      all(k in rul for k in ("meta", "conflict_audit", "mint_ruling",
                             ...
                             "non_goals")))
m = rul["meta"]
check("A2 ruling attribution (session 66, operator-commissioned, 2026-09-25)",
      m.get("session") == 66 and m.get("recorded_date") == "2026-09-25"
      and "commission batch 11" in (m.get("commissioned_by") or ""))
...
# ---------------------------------------------------------------------------
# B. conflict audit re-run against the PRE-batch-11 store
# ---------------------------------------------------------------------------
pre_nodes = []
...
b11_codes = {x["code"] for x in nodes_doc["nodes"]} - pre_codes
...
terms = rul["conflict_audit"]["s4_def_candidate_terms"]
matched = [t for t in terms if t in store_blob or t in endpoints]
check("B1 the recorded 72-match audit reproduces on the pre-batch-11 store "
      "(registry state the ruling was recorded against)",
      len(matched) == 72, f"matched = {len(matched)}")
...
check("C max_boundary_edges honoured (exactly 12 sanctioned targets)",
      len(targets) == 12
      and rul["boundary_edge_ruling"]["max_boundary_edges"] == 12)

# ---------------------------------------------------------------------------
# D. discipline invariants
# ---------------------------------------------------------------------------
expected_nodes = 188 + (5 if B11_RECORD.exists() else 0)
expected_edges = 459 + (29 if B11_RECORD.exists() else 0)
expected_partof = 199 + (12 if B11_RECORD.exists() else 0)
check("D1 the ruling mints no node beyond the sanctioned batch-11 authoring",
      len(live_codes) == expected_nodes,
      f"live = {len(live_codes)}, expected = {expected_nodes}")
check("D2 the ruling mints no edge beyond the sanctioned batch-11 authoring",
...
# session-67 re-anchor (2026-09-25, dated): the batch-11 §18 application (17 operator
# promotions, c11_batch11_verdicts) moved the semantic HV count 255 -> 272; the protected
# property is unchanged.
check("D3 semantic HV at the operator-promoted count (272; the batch-11 "
      ...
      "via §18 — the 17 batch-11 authored edges were promoted by the "
      "operator's verdicts via §18 at session 67; PART_OF HV rides "
...
if fails:
    ...
    raise SystemExit(1)
print(f"c11_batch11_boundary_check: ALL PASS — the cross-slice boundary "
      f"ruling (session 66) is schema-valid, its 72-match dispositioned "
      f"audit reproduces on the pre-batch-11 store, every sanctioned "
      f"boundary target exists with exact ownership, and the ruling mints "
      f"and promotes nothing beyond the sanctioned batch-11 "
