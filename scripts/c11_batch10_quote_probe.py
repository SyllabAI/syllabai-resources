#!/usr/bin/env python3
"""T-C11 session 62 — batch-10 quote probe (fail-closed).

Walks scripts/c11_batch10_decisions.yaml and verifies EVERY evidence quote
(nodes + edges) matches its cited file under the batch-8 G03/c11.4
normalization convention (c11_batch8_quote_probe.py's norm(): NFC, markdown
...
def main() -> int:
    ...
    for n in doc["nodes"]:
        anchor_groups = [("attachment", n.get("spec_points") or [])]
        if n.get("evidence"):
            anchor_groups.append(("node-evidence",
                                  [{"evidence": n["evidence"]}]))
        for sp in n.get("spec_points") or []:
    ...
    if fails:
        ...
        for f in fails:
            print("  -", f)
        return 1
    print("c11_batch10_quote_probe: ALL PASS — every quote verbatim under "
