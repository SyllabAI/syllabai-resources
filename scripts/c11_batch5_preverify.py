#!/usr/bin/env python3
"""T-C11 session 55 — pre-write verification of the batch-5 decision record
(scripts/c11_batch5_decisions.yaml): every evidence/remediation quote must
...
(The c11_batch4_preverify.py pattern, batch-5-scoped, C28-registry-resolved.)

Session-51 note (still true): this is a PRE-write tool. After the batch-5
...
REPO = HERE.parent
DEC = HERE / "c11_batch5_decisions.yaml"
...
def main() -> int:
    ...
    ruling = yaml.safe_load(
        (HERE / "c11_batch5_boundary_ruling.yaml").read_text(encoding="utf-8"))
    sanctioned = {t["target"] for t in
                  ruling["boundary_edge_ruling"]["sanctioned_targets"]}
    boundary_edges = [e for e in dec["edges"]
                      if e["target"] in sanctioned
                      and e["source"] in own_codes]
