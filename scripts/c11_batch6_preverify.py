#!/usr/bin/env python3
"""T-C11 session 57 — pre-write verification of the batch-6 decision record
(scripts/c11_batch6_decisions.yaml): every evidence/remediation quote must
...
(The c11_batch4_preverify.py pattern, batch-6-scoped, C28-registry-resolved.)

Session-51 note (still true): this is a PRE-write tool. After the batch-6
record enters the registry and the graph is generated, the "edge re-uses an
existing identity" failures it reports against the LIVE store are the batch's
OWN edges (self-match) — expected post-generation, not a defect. The
authoring-time run over the pre-registry state must be fully green.
"""
...
REPO = HERE.parent
DEC = HERE / "c11_batch6_decisions.yaml"
...
def main() -> int:
    ...
    def check_anchor(anchor, where, require_scoped_note=True):
        ...
        rel = GP.resolve_rel(anchor.get("file", ""))
    ...
    live_nodes = yaml.safe_load(GP.store("concepts").read_text(encoding="utf-8"))
    live_by_code = {n["code"]: n for n in live_nodes["nodes"]}
    own_codes = {n["code"] for n in dec["nodes"]}
    practicals = yaml.safe_load(GP.store("practicals").read_text(encoding="utf-8"))
    ...
    for e in dec["edges"]:
        ...
        for a in e.get("evidence") or []:
            if a.get("kind") == "NOTE":
                mapped = tc10.get(GP.resolve_rel(a.get("file", "")), set())
    ...
    codes = {n["code"] for n in live_nodes["nodes"]}
    for e in dec["edges"]:
    ...
    edges_doc = yaml.safe_load(GP.store("concept_edges").read_text(encoding="utf-8"))
    ...
    # scoped-attachment check (G05 shape) + the ruling's sanctioned boundary set
    scope = set(dec["meta"]["scope"]["spec_points"])
    ruling = yaml.safe_load(
        (HERE / "c11_batch6_boundary_ruling.yaml").read_text(encoding="utf-8"))
    sanctioned = {t["target"] for t in
                  ruling["boundary_edge_ruling"]["sanctioned_targets"]}
    # session-57 adaptation: the batch-6 sanctioned set includes the TWO
    # deferral-closure edges whose TARGETS are batch-6 mints (source owned by
    # the batch-5 record) — a boundary edge is any sanctioned-set endpoint
    # whose other endpoint is NOT owned by this record (both-own endpoints
    # would be in-slice edges, not boundary edges).
    def _other_own(e):
        return e["source"] in own_codes and e["target"] in own_codes
    boundary_edges = [e for e in dec["edges"]
                      if (e["target"] in sanctioned or e["source"] in sanctioned)
                      and not _other_own(e)]
    if len(boundary_edges) > ruling["boundary_edge_ruling"]["max_boundary_edges"]:
        fails.append(f"boundary-edge count {len(boundary_edges)} exceeds the "
                     f"ruling's sanctioned maximum")
    for e in boundary_edges:
        if e["relation"] != "REQUIRES_PREREQUISITE":
            fails.append(f"boundary edge {e['source']} -> {e['target']} is "
                         f"{e['relation']}, ruling sanctions REQUIRES_PREREQUISITE only")
    for n in dec["nodes"]:
        for att in n.get("spec_points") or []:
    ...
    print(f"pre-verification: {n_checks} quote anchors checked; "
          f"{len(boundary_edges)} boundary edges == the sanctioned set")
    if fails:
