#!/usr/bin/env python3
"""T-C11 session 62 — batch-9 preverify (fail-closed, run BEFORE the
registry grows; the c11_batch8_preverify.py pattern).

Machine-verifies the batch-9 decision record's structure and its boundary
discipline against the LIVE pre-batch-9 store and the ruling:
...
# 1. record shape
check("record shape: 15 nodes (14 CONCEPT + 1 MISCONCEPTION)",
      len(b_nodes) == 15
      and sum(1 for x in b_nodes if x["family"] == "CONCEPT") == 14
      and sum(1 for x in b_nodes if x["family"] == "MISCONCEPTION") == 1)
check("record shape: 20 edges (18 RP + 1 WAP + 1 RB)",
      len(b_edges) == 20
      and sum(1 for e in b_edges if e["relation"] == "REQUIRES_PREREQUISITE") == 18
...
check("record shape: 21 command kinds (one per authorable SP)",
      len(b_kinds) == 21
      and len({k["code"] for k in b_kinds}) == 21)
...
check("boundary discipline: exactly 5 edges target EXISTING owners",
      len(boundary_edges) == 5
...
check("misconception pairing: the WAP target == the REMEDIATED_BY target "
      ...
      and wap[0]["source"] == "4CH1-MIS-KEROSENE-DOUBLE-BONDS")

# 5. the live store must be the sanctioned pre-batch-9 state
check("pre-state: live store 165 nodes / 384 edges (163 PART_OF)",
...
check("pre-state: 216 semantic HUMAN_VALIDATED (the batch-8 §18 state)",
      hv == 216)
check("pre-state: no batch-9 code already live (the registry has not "
      "grown)",
...
if fails:
    ...
    for f in fails:
        print("  -", f)
    raise SystemExit(1)
print(f"c11_batch9_preverify: ALL PASS ({n} checks) — record shape, SP "
      f"coverage, boundary discipline, misconception pairing and the "
      f"pre-batch-9 store state all verified; the registry may grow")
