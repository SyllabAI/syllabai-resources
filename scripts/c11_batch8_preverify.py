#!/usr/bin/env python3
"""T-C11 session 61 — pre-write verification of the batch-8 decision record
(scripts/c11_batch8_decisions.yaml): every evidence/remediation quote must
...
(The c11_batch7_preverify.py pattern, batch-8-scoped, C28-registry-resolved.)

Session-51 note (still true): this is a PRE-write tool. After the batch-8
...
REPO = HERE.parent
DEC = HERE / "c11_batch8_decisions.yaml"
...
def main() -> int:
    ...
    ruling = yaml.safe_load(
        (HERE / "c11_batch8_boundary_ruling.yaml").read_text(encoding="utf-8"))
    sanctioned = {t["target"] for t in
                  ruling["boundary_edge_ruling"]["sanctioned_targets"]}
    # session-61 adaptation: the batch-8 sanctioned set is BOTH-EXISTING-OWNER
    # targets (no deferral closures this batch) — a boundary edge is any
    # sanctioned-set endpoint whose other endpoint is NOT owned by this record.
    def _other_own(e):
