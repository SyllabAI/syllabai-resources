...
Real-repo smoke (read-only):
  R1  `list` exits 0 on the live store state and reports the session-52
      surface (0 actionable / 5 not-actionable / 118 promotions
      recorded) — the operator's batch-3 verdicts were applied (session 52,
      the practical-review policy) and the §18 actionable surface is
      empty again until the next batch's gate
...
def make_fixture(base: Path):
    ...
    (base / "scripts" / "c11_batch9_decisions.yaml").write_text(
        yaml.safe_dump({"meta": {"task": "T-C11", "stage": "s16-batch-9",
                       ...
                       allow_unicode=True, sort_keys=False, width=100),
        encoding="utf-8")
    # session-64: the registry grows by the (empty) batch-10 member
    (base / "scripts" / "c11_batch10_decisions.yaml").write_text(
        yaml.safe_dump({"meta": {"task": "T-C11", "stage": "s16-batch-10",
                                   "extraction_pass": "c11-s16-batch-10",
                       ...
                       allow_unicode=True, sort_keys=False, width=100),
        encoding="utf-8")
    # session-66: the registry grows by the (empty) batch-11 member
    (base / "scripts" / "c11_batch11_decisions.yaml").write_text(
        yaml.safe_dump({"meta": {"task": "T-C11", "stage": "s16-batch-11",
                                   "extraction_pass": "c11-s16-batch-11",
                                   "generated_date": "2026-09-25",
                                   "model_version": "GLM (Super Z agent, z.ai)",
                                   "curriculum_code": "4CH1-2017",
                                   "scope": {"spec_points": [],
    ...
    (base / "scripts" / "c11_batch11_decisions.yaml").write_text(
        yaml.safe_dump({"meta": {"task": "T-C11", "stage": "s16-batch-11",
                       ...
                       allow_unicode=True, sort_keys=False, width=100),
        encoding="utf-8")
    # session-59: the registry grows by the (empty) batch-7 member
    (base / "scripts" / "c11_batch7_decisions.yaml").write_text(
        yaml.safe_dump({"meta": {"task": "T-C11", "stage": "s16-batch-7",
                                   "extraction_pass": "c11-s16-batch-7",
...
def main() -> int:
    ...
    try:
        ...
        # promotes nothing).
        # session-56: the batch-5 verdicts were APPLIED (18 §18 promotions,
        ...
        # session-59 re-anchor (dated, protective intent unchanged): the
        ...
        # session-62 re-anchor (2026-09-24, dated, protective intent
        # unchanged): the batch-8 verdicts were APPLIED (10 §18 promotions,
        ...
        # session-64 re-anchor (2026-09-25, dated): the batch-10 authored-to-
        # gate record (19 authored semantic edges, zero promotions) joins the
        # registry, so the actionable surface is 19 again (the batch-10
        # authored set) / 5 not-actionable (the frozen pilot HOLDs) / 236.
        # session-65 re-anchor (2026-09-25, dated, protective intent
        # unchanged): the batch-10 verdicts were APPLIED (19 §18 promotions,
        # c11_batch10_verdicts); promo_count 236 -> 255 (28+28+23+39+35+18
        # +16+19+10+20+19); the actionable surface is empty again until
        # batch 11's gate (the not-actionable rows stay 5).
