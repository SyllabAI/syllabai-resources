...
def mut_07_forged_promotion(g: Path):
    ...
    # session-49 note: after the batch-2 authoring the first SUGGESTED
    # semantic edge is a batch-2 edge (sorted order: CON-AR -> CON-ISOTOPES,
    # clean SUGGESTED awaiting its operator gate) — same corruption class.
    # session-50 note: after the batch-2 verdict application that edge is
    # HUMAN_VALIDATED; the only SUGGESTED semantic edges left are again the
    # 3 pilot operator HOLDs (session-48 note applies once more) — same
    # corruption class (unbacked HUMAN_VALIDATED on a SUGGESTED edge).
    # session-52 note: after the batch-3 §18 application (39 promotions) the
    # state is unchanged in kind — the only SUGGESTED semantic edges are
    # again the 3 pilot operator HOLDs — same corruption class.
    # session-53 note: after the batch-4 authoring the first SUGGESTED
    ...
    # corruption class (unbacked HUMAN_VALIDATED on a SUGGESTED edge).
    # session-56 note: after the batch-5 §18 application (18 promotions,
    # c11_batch5_verdicts) the only SUGGESTED semantic edges are AGAIN the
    # 3 pilot operator HOLDs (session-48/50/52/54 notes apply once more) —
    # same corruption class (unbacked HUMAN_VALIDATED on a SUGGESTED edge).
    # session-58 note: after the batch-6 §18 application (16 promotions,
    # c11_batch6_verdicts) the only SUGGESTED semantic edges are AGAIN the
    ...
    # more) — same corruption class (unbacked HUMAN_VALIDATED on a
    # SUGGESTED edge).
    # session-62 note: after the batch-8 §18 application (10 promotions,
    # c11_batch8_verdicts) the only SUGGESTED semantic edges are AGAIN the
