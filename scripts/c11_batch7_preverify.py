#!/usr/bin/env python3
"""T-C11 session 59 — pre-write verification of the batch-7 decision record
(scripts/c11_batch7_decisions.yaml): every evidence/remediation quote must
...
(The c11_batch6_preverify.py pattern, batch-7-scoped, C28-registry-resolved.)

Session-51 note (still true): this is a PRE-write tool. After the batch-7
...
REPO = HERE.parent
DEC = HERE / "c11_batch7_decisions.yaml"
...
def main() -> int:
    ...
    ruling = yaml.safe_load(
        (HERE / "c11_batch7_boundary_ruling.yaml").read_text(encoding="utf-8"))
    sanctioned = {t["target"] for t in
                  ruling["boundary_edge_ruling"]["sanctioned_targets"]}
    # session-59 adaptation: the batch-7 sanctioned set is BOTH-EXISTING-OWNER
